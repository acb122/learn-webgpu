...
async function main() {
  ...
  document.addEventListener('keydown', (e) => {
    const cameraSpeed = deltaTime / 1e3; // adjust accordingly
    if (e.key == 'w') {
    ...
    if (e.key == 'a') {
      cameraPos = cameraPos.sub(glm.normalize(glm.cross(cameraFront, cameraUp)).mul(cameraSpeed));
    } if (e.key == 'd') {
      cameraPos = cameraPos.add(glm.normalize(glm.cross(cameraFront, cameraUp)).mul(cameraSpeed))
    }
  })
  ...
  function loop() {
    ...
    deltaTime = currentFrame - lastFrameTs;
    lastFrameTs = currentFrame;




    let view = glm.lookAt(cameraPos, cameraPos.add(cameraFront), cameraUp);
